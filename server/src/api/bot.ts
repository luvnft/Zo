...
export default class Bot {
  ...
  private async replyStartMessage(chatId: number): Promise<void> {
    await this.bot!.sendMessage(chatId, 'Welcome to ZO House Stay Booking Bot. THIS IS NOT REAL ZO BOOKING BOT. This is just a sample app built for a hackathon and is in no way affiliated with the real Zo Houses or Zo World. The payments made here are not real, this bot will never ask for real money.', {
      reply_markup: {
        inline_keyboard: [
          [{
            text: `🦄 Open ${this.config.appName}`,
  ...
  private async replyHelpMessage(chatId: number): Promise<void> {
    await this.bot!.sendMessage(chatId, 'This is an example bot.', {
      reply_markup: {
        inline_keyboard: [
          [{
            text: `🦄 Open ${this.config.appName}`,
