...
<template>
  <div class="home-page">
    <Placeholder ref="landing" class="landing" title="Zo House"
      caption="Experience culturally rich, luxury clubhouses exclusively for founder members." :class="{
      ...
      <template #picture>
        <div class="landing-picture">
          <Transition name="switch">
            <img v-if="!isLoading" src="/zo.png" aria-hidden="true" width="68" style="border-radius: 10px;">
            <Lottie v-else-if="animationData !== null" :animation-data="animationData" width="110px" height="110px" />
          </Transition>
        </div>
      </template>
    </Placeholder>
    <Sections>
      <Section ref="searchSettings" padded>
        <List with-background standalone>
          <ListItem label="Check In Date">
            <template #right>
              <DatePickerCompact :value="trip.startDate" @click="onStartDateClick" />
            </template>
          </ListItem>
          <ListItemExpandable :opened="startDatePickerShowed">
            <DatePicker ref="startDatePicker" :min-date="new Date()" @date-pick="(date) => setStartDate(date)" />
          </ListItemExpandable>
          <ListItem label="Check Out Date">
            <template #right>
              <DatePickerCompact :value="trip.endDate" @click="onEndDateClick" />
            </template>
          </ListItem>
          <ListItemExpandable :opened="endDatePickerShowed">
            <!-- Max date is start + 60 -->
            <DatePicker ref="endDatePicker" :min-date="trip.startDate || new Date()"
              :max-date="new Date(trip.startDate.getTime() + 60 * 24 * 60 * 60 * 1000)" :value="trip.endDate"
              @date-pick="(date) => setEndDate(date)" />
          </ListItemExpandable>
          <ListItem label="Location" right-icon="chevron-right" :right-icon-label="location?.title" to="/location" />
        </List>
      </Section>
      <Section v-if="isLoading === false" padded>
        <List gapped class="results">
          <template v-for="(hotel, index) in result" :key="'hotel:' + hotel.id">
            <ListCard v-if="index === 3 || index === 5" :title="index === 3 ? 'Users choice' : 'Hotel of the year'"
              :picture="hotel.picture" :picture-thumb="(hotel.pictureThumb as string)">
              <template #visible>
                <ListItem :id="1"
                  :avatar="{src: hotel.picture, placeholder: hotel.title, pictureThumb: hotel.pictureThumb}"
                  :title="hotel.title" :subtitle="hotel.subtitle" nowrap>
                  <template #right>
              ...
              <template #collapsed>
                <Sections>
                  ...
                  <Section padded>
                    <List>
                      <ListItem :id="hotel.id" transaction-icon="clock-fill" title="Check rooms"
                        :subtitle="`There are ${hotel.rooms.length} rooms available`" :to="`/hotel/${hotel.id}`"
                        right-icon="chevron-right" standalone />
                    </List>
                  </Section>
                </Sections>
              </template>
            </ListCard>
            <ListItem v-else :id="hotel.id"
              :avatar="{src: hotel.picture, placeholder: hotel.title, pictureThumb: hotel.pictureThumb}"
              :title="hotel.title" :subtitle="hotel.subtitle" :to="`/hotel/${hotel.id}`" big-avatar standalone
